...
public:
	...
	*/
	...
	std::unordered_map<int, std::shared_ptr<FactoryRoot>> m_factories;
	//заменил std::map на atd::unordered_map для более быстрого поиска ключа: O(log(n)) против O(1)
	...
	void RegisterFactory() {
		...
		[](std::shared_ptr<TArguments>... arguments) -> std::shared_ptr<TInterface> {
					return std::make_shared<TConcrete>(
						...
						std::shared_ptr<TArguments>(arguments)...);
					//так как std::forward здесь ничего не делает ввиду передачи аргументов в лямбду по значению, я его убрал
		}));
	}
